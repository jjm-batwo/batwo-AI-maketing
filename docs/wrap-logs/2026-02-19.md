...
### 변경 파일 (26개, 신규 1개 + 수정 25개)
| 파일 | 변경 유형 | 설명 |
|------|----------|------|
| `src/infrastructure/external/meta-pixel/MetaPixelClient.ts` | 수정 | Meta API v18.0 → v25.0 |
| `src/infrastructure/external/meta-pixel/CAPIClient.ts` | 수정 | Meta API v18.0 → v25.0 |
| `src/app/(dashboard)/settings/meta-connect/page.tsx` | 수정 | OAuth URL v18.0 → v25.0 |
| `src/presentation/components/common/FacebookSDK.tsx` | 수정 | FB SDK v21.0 → v25.0 |
| `scripts/exchange-token.ts` | 수정 | accessToken encryptToken() 래핑 |
| `scripts/setup-warmup.ts` | 수정 | accessToken encryptToken() 래핑 |
| `scripts/meta-warmup.ts` | 수정 | account.accessToken safeDecryptToken() 래핑 |
| `prisma/seed.ts` | 수정 | demo 토큰 encryptToken() 래핑 |
| `campaigns/[id]/edit/page.tsx` | 수정 | revalidate: 0 → 60 + tags + metadata |
| `campaigns/[id]/analytics/page.tsx` | 수정 | revalidate: 0 → 60 + tags + metadata |
| `settings/meta-pages/page.tsx` | 수정 | Image sizes + 에러 다크모드 색상 |
| `src/presentation/hooks/useTeams.ts` | 수정 | staleTime 추가 (2분/1분) |
| `src/app/api/reports/[id]/route.ts` | 수정 | DELETE에 admin-dashboard revalidateTag |
| `admin/analytics/page.tsx` | 수정 | no-store → ISR 300s + metadata |
| `admin/users/[id]/page.tsx` | 수정 | Image sizes + metadata |
| `src/app/(admin)/loading.tsx` | **신규** | Admin 로딩 스켈레톤 |
| `campaigns/page.tsx` | 수정 | metadata 추가 |
| `campaigns/[id]/page.tsx` | 수정 | metadata 추가 |
| `reports/page.tsx` | 수정 | metadata 추가 |
| `reports/[id]/page.tsx` | 수정 | metadata 추가 |
| `reports/sample/page.tsx` | 수정 | metadata 추가 |
| `settings/page.tsx` | 수정 | metadata 추가 |
| `settings/teams/page.tsx` | 수정 | metadata 추가 |
| `settings/teams/[id]/page.tsx` | 수정 | metadata 추가 |
| `admin/page.tsx` ~ `admin/settings/page.tsx` | 수정 | metadata 추가 (5개) |

### 핵심 변경사항
- [검증] **verify-implementation 8개 이슈 수정** — Meta API v25.0 4건 + 토큰 암복호화 4건
- [최적화] **/full-audit 실행** — 4개 영역 57건 식별 (P0=13, P1=24, P2=20)
- [P0] **ISR 캐싱** — edit/analytics revalidate: 0→60, Image sizes, 다크모드 색상
- [P1] **캐시 최적화** — useTeams staleTime, reports→admin-dashboard, admin analytics ISR, admin loading.tsx
- [P2] **메타데이터** — 17개 RSC 페이지 Metadata export 일괄 추가

### 해결된 이슈
- MetaPixelClient/CAPIClient/FacebookSDK/OAuth의 구 API 버전 잔존
- scripts/seed 토큰 평문 저장
- edit/analytics ISR 미적용, useTeams 리페치 과다, reports DELETE 캐시 미갱신
- Admin analytics no-cache, Admin loading 빈 화면
- 17개 RSC 페이지 브라우저 탭 제목 누락

### 미해결 사항
- P0-2: Vercel 프로덕션 배포
- P0-3: Facebook OAuth 프로덕션 전환
...
### 우선순위 높음 (P0)
- [ ] Vercel 프로덕션 배포 — 환경변수, DB 마이그레이션 (관련: `vercel.json`, `.env`)
- [ ] Facebook OAuth 프로덕션 전환 — 앱 Live 모드 (관련: `src/infrastructure/auth/auth.ts`)

### 우선순위 보통 (P1)
- [ ] 커밋 생성 — 세션4+5 변경사항 (~48개 파일)
- [ ] 다크모드 수정 완료 — 플랜 Part A 잔여 항목 (관련: `.claude/plans/curious-discovering-cat.md`)
- [ ] 캠페인 테이블 UI — Meta Ads Manager 스타일 CampaignTable (관련: 플랜 Part B)

---

### 백로그 변동
- 신규 이관: 3개 (P2a: 0개, P2b: 3개)
- 완료 제거: 0개
...
| 파일 | 변경 유형 | 설명 |
|------|----------|------|
| `settings/meta-connect/page.tsx` | 수정 | `bg-orange-100` → `bg-orange-500/15` |
| `dashboard/KPIChart.tsx` | 수정 | 바 차트 툴팁 `bg-gray-900` → `bg-popover text-popover-foreground` |
| `dashboard/AnomalyAlert.tsx` | 수정 | SEVERITY_CONFIG 3개 + PRIORITY_COLORS 3개 다크모드 호환 |
| `dashboard/CampaignSummaryTable.tsx` | 수정 | COMPLETED 점 `bg-gray-400` → `bg-muted-foreground` |
| `campaign/CampaignTable.tsx` | 수정 | COMPLETED 점 `bg-gray-400` → `bg-muted-foreground` |
| `campaign/CampaignList.tsx` | 수정 | 스켈레톤 `bg-gray-100` → `bg-muted` |
| `campaigns/[id]/CampaignDetailClient.tsx` | 수정 | statusConfig 5개 상태 다크모드 호환 |
| `campaign/AdList.tsx` | 수정 | statusLabels 4개 상태 다크모드 호환 |
| `campaign/AdSetList.tsx` | 수정 | statusLabels 4개 상태 다크모드 호환 |
| `campaign/BudgetAlertBadge.tsx` | 수정 | 프로그레스바 `bg-gray-200` → `bg-muted`, 마커 `bg-gray-600` → `bg-muted-foreground` |
| `report/ReportDetail.tsx` | 수정 | 스켈레톤 `bg-gray-200` → `bg-muted` (replace_all) |
| `report/ReportList.tsx` | 수정 | 스켈레톤 `bg-gray-100` → `bg-muted` + statusConfig 3개 상태 |
| `alerts/NotificationCenter.tsx` | 수정 | 스켈레톤 `bg-gray-100` → `bg-muted` |
| `ab-test/ABTestList.tsx` | 수정 | 스켈레톤 `bg-gray-100` → `bg-muted` |
| `ab-test/ABTestCard.tsx` | 수정 | statusConfig 4개 상태 다크모드 호환 |
| `ab-test/CreateABTestDialog.tsx` | 수정 | 컨트롤 배지 `bg-gray-200` → `bg-muted` |
| `quota/QuotaStatusBadge.tsx` | 수정 | 배지 `bg-gray-100 text-gray-700` → `bg-muted text-muted-foreground` |
| `onboarding/OnboardingWizard.tsx` | 수정 | 프로그레스바 `bg-gray-100` → `bg-muted` |
| `campaign/CampaignCreateForm/index.tsx` | 수정 | 프로그레스바 `bg-gray-100` → `bg-muted` |
| `CampaignSummaryTable.test.tsx` | 수정 | `bg-gray-400` → `bg-muted-foreground` 어설션 수정 |

### 핵심 변경사항
- [디자인] **다크모드 가시성 일괄 수정 (Plan Part A)** — 20개 컴포넌트에서 하드코딩 라이트 전용 색상을 시맨틱 토큰으로 교체
- [디자인] **상태 배지 통일 패턴** — 모든 컴포넌트의 statusConfig를 `bg-{color}-500/15 text-{color}-500` 패턴으로 통일
- [디자인] **스켈레톤/프로그레스바 통일** — `bg-gray-100/200` → `bg-muted`, `bg-gray-400/600` → `bg-muted-foreground`
- [버그] **테스트 실패 수정** — CampaignSummaryTable.test.tsx의 `bg-gray-400` 어설션을 `bg-muted-foreground`로 수정
- [확인] **Part B 이미 완료** — CampaignTable.tsx + CampaignsClient.tsx 연동이 이전 세션에서 완료 확인

### 해결된 이슈
- 다크모드에서 `bg-gray-100/200` 스켈레톤이 보이지 않는 문제 → `bg-muted` 토큰
- 다크모드에서 `bg-{color}-100 text-{color}-700` 상태 배지 가독성 저하 → `bg-{color}-500/15` 패턴
- AnomalyAlert SEVERITY_CONFIG/PRIORITY_COLORS 다크모드 미지원 → `dark:text-{color}-400` 추가
- KPIChart 바 차트 툴팁 다크모드에서 가시성 저하 → `bg-popover` 시맨틱 토큰
- 1개 테스트 실패 (CampaignSummaryTable COMPLETED dot 색상 어설션)

### 미해결 사항
- AI 컴포넌트(FirstUseGuide, AIFeatureTour 등)에 `bg-gray-*` 잔존하나 `dark:` 변형 이미 적용됨 → 저우선순위
- Pixel 관련 컴포넌트(PixelSelector, PixelStatus)에 `bg-gray-*` 잔존 → 저우선순위

---

## 규칙 업데이트

- 규칙 업데이트 불필요 (다크모드 색상 패턴은 이미 메모리에 기록됨)
...
### 우선순위 보통 (P1)
- [ ] 커밋 생성 — 세션4~6 변경사항 (~65개 파일) (관련: `feat/design-renewal` 브랜치)
- [ ] 브라우저 시각적 검증 — 대시보드/설정/캠페인 페이지 다크모드 가시성 확인
