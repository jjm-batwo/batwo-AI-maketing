...
버그와 싸우는 체계적인 분석 프레임워크입니다.

#### 1단계: 문제를 한 문장으로 정의
**왜 필요한가?** 버그 발생 시 당황하면 사고와 시야가 좁아집니다. 현재 무슨 문제를 푸는지 명확히 인지해야 합니다.

| ❌ 나쁜 예 | ✅ 좋은 예 |
|-----------|-----------|
| "쿠폰이 이상해요" | "쿠폰 로직에 따른 금액 계산 결과가 정확하지 않다" |
| "로그인이 안 돼요" | "OAuth 토큰 갱신 시 세션이 유지되지 않는다" |

#### 2단계: 올바른 동작 정의 (Given-When-Then)
**핵심 질문**: "올바르게 동작한다면 어떤 일이, 어떤 순서로 벌어져야 하는가?"

> 정의할 수 없다면 문제를 제대로 해결하는 것도 불가능합니다.

**예시 - 쿠폰 할인 버그:**
```
Given: 일반 회원 + 10% 할인 쿠폰 + 프로모션 기간 아님
When:  장바구니 추가 → 10% 쿠폰 적용 → 결제 시도
Then:  상품 원가의 10%가 할인된 금액으로 결제
       추가 프로모션 할인은 적용되지 않음
```

#### 3단계: 최소 재현 환경 구축
재현 환경 구축이 안 되면 문제 해결이 어렵습니다.

**체크리스트:**
- [ ] 문제 발생 시점 확인
- [ ] 배포 전후 변경사항 확인 (`git diff`, `git log`)
- [ ] 내 환경에서 직접 재현 시도
- [ ] 문제 부분을 격리 (최소 재현 케이스)
- [ ] 벌어지는 일들을 유심히 관찰

**관찰 포인트:**
- 에러 메시지 내용, 출처, 스택 트레이스
- 정상 환경과 재현 환경의 차이 패턴 파악

#### 4단계: 원인 옵션 나열
재현 환경과 정상 환경의 차이를 만드는 원인을 나열합니다.

- 추상적이든 구체적이든 떠오르는 대로 적기
- "이 부분이 문제다", "이 요소가 문제다", "이게 이렇게 되어야 하는데..."
...
#### 5단계: 가설 검증
옵션을 **검증 가능한 가설**로 문장화하고 테스트합니다.

- 작은 변경을 가하면서 가설대로 현상이 변하는지 관찰
- **시간 제한 설정**: 넘어가면 빠르게 동료 도움 받기
- 각 가설에 대해 실패 테스트 작성 → 수정 → 통과 확인
