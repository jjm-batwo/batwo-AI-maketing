...
### 현재 상태 분석
- ✅ `IKPIRepository` - aggregateByCampaignId 존재 (일별 그룹화 없음)
- ✅ `PrismaKPIRepository` - 총합 집계만 구현
- ❌ `GetDashboardKPIUseCase` - chartData 없음
- ❌ API Route - chartData 응답 없음
- ✅ `KPIChart` 컴포넌트 - DataPoint[] 수신 가능
- ❌ Dashboard Page - chartData 빈 배열 사용
...
**테스트 케이스**:
- [ ] chartData 반환 확인 (일별 데이터 포인트)
- [ ] ROAS 계산 정확성 (revenue/spend)
- [ ] 빈 chartData 처리 (캠페인 없음)
- [ ] 날짜 범위 필터링 동작
...
- [ ] ROAS 계산 로직 추출 (value object 또는 helper)
- [ ] 날짜 포맷팅 유틸리티 사용
- [ ] 테스트 유지 확인
...
**테스트 케이스**:
- [ ] GET /api/dashboard/kpi 응답에 chartData 포함
- [ ] chartData 배열 구조 검증
...
- [ ] getDailyAggregates 메서드 구현 및 테스트 통과
- [ ] GetDashboardKPIUseCase에서 chartData 반환
- [ ] API 응답에 chartData 포함
- [ ] 대시보드 차트에 실제 KPI 추이 표시
- [ ] 모든 테스트 통과 (목표: 360개+)
- [ ] Application 레이어 커버리지 ≥90%
