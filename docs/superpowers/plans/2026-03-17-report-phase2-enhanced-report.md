...
- [ ] **1.1** `src/application/dto/report/EnhancedReportSections.ts` 신규 파일 생성 -- 9개 섹션 타입 정의
...
- [ ] **1.2** `src/application/dto/report/ReportDTO.ts` 수정 -- `EnhancedReportSections` 필드 추가
...
- [ ] **1.3** `src/application/dto/report/__tests__/EnhancedReportSections.test.ts` 신규 -- 타입 호환성 테스트
...
- [ ] **2.1** `prisma/schema.prisma` -- Report 모델에 `enrichedData` 필드 추가
...
- [ ] **2.3** `src/domain/entities/Report.ts` -- enrichedData 속성 추가
...
- [ ] **2.4** 기존 Report 테스트 파일에 enrichedData 관련 테스트 추가
...
- [ ] **4.1** `src/domain/value-objects/FunnelStage.ts` 신규 -- 퍼널 스테이지 정의
...
- [ ] **6.1** `BaseReportGenerationUseCase` 생성자에 `EnhancedReportDataBuilder` 추가
...
- [ ] **6.2** `execute()` 메서드에 Step 3.5 추가 (buildCampaignSections 후, markAsGenerated 전)
...
- [ ] **6.4** GenerateWeeklyReportUseCase 생성자 업데이트
...
- [ ] **6.5** 기존 BaseReportGenerationUseCase 테스트에 enrichedData 빌드 테스트 추가
...
- [ ] **7.1** `src/lib/sample-enhanced-report-data.ts` 신규 -- 9개 섹션 목업 데이터
...
- [ ] **7.2** `src/app/api/reports/sample/route.ts` 수정 -- enhanced 샘플 데이터 옵션 추가
...
- [ ] **7.3** `src/app/api/reports/sample/download/route.ts` 수정 -- enhanced PDF 분기
...
- [ ] **8.1** `SummaryCard.tsx` -- 변화율 포함 KPI 카드
...
- [ ] **8.3** `FatigueMatrix.tsx` -- 소재 피로도 매트릭스
...
- [ ] **8.4** `FunnelChart.tsx` -- ToFu/MoFu/BoFu 퍼널 도식
...
- [ ] **8.5** `FormatComparisonChart.tsx` -- 포맷별 성과 비교 차트
...
- [ ] **8.6** `PriorityActionCard.tsx` -- 우선순위별 액션 카드
...
- [ ] **9.2** 기존 `formatCurrency` 수정 (B2) -- `WeeklyReportTemplate.tsx`와 `BaseReportTemplate.tsx`
...
- [ ] **10.1** `ReportPDFGenerator.generateWeeklyReport` 수정 -- enrichedData 기반 템플릿 분기 (B1)
...
- [ ] **11.1** `src/lib/di/types.ts`에 Phase 2 DI 토큰 추가
...
- [ ] **11.2** `src/lib/di/modules/report.module.ts` 확장
...
- [ ] **12.1** EnhancedWeeklyReportTemplate 렌더링 테스트
...
- [ ] **12.2** ReportPDFGenerator 분기 테스트
