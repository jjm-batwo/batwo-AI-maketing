...
const KNOWLEDGE_DOCUMENTS: KnowledgeDocument[] = [
  ...
  // 캠페인 구조
  {
    ...
    title: '캠페인 구조 최적화와 학습 단계 — 광고가 멈춰버렸을 때, 광고 세트가 너무 많을 때',
    content: 'Advantage+ 캠페인은 Lattice 알고리즘에 최적화된 구조입니다. 캠페인 수를 최소화하고(목표당 1-2개), 광고 세트를 통합해야 합니다. 광고 세트가 너무 많은 것 같다면 구조를 단순화하세요. 파편화된 구조(10개 이상 세트)는 학습 데이터가 분산되어 최적화가 느려집니다. 광고가 멈춰버렸거나 학습 단계에서 벗어나질 못할 때는 구조 통합과 예산 집중이 필요합니다. 광범위 타겟팅(Broad Targeting)이 수동 타겟팅보다 효과적입니다. 일일 예산 $50 이상이 학습 단계 탈출의 최소 조건입니다.',
  },
  // 추적 건강성
  {
  ...
  // 예산 최적화
  {
    ...
    title: '광고 예산 배분과 ROAS 개선 — 돈은 쓰는데 효과가 없을 때',
    content: 'ROAS 기반 배분: ROAS가 높은 캠페인에 예산 집중. CPA 기반 배분: 목표 CPA 이하인 캠페인에 예산 우선 할당. 돈은 쓰는데 효과가 없다면 예산 배분을 재검토하세요. 예산 증액은 10-20% 점진적으로, 급격한 증액은 학습 단계를 초기화할 수 있습니다. 안정화 기간(Stabilization Window)을 반드시 확보하세요. 광고비 대비 매출(ROAS)이 2.5배 이상이면 Good, 4배 이상이면 Excellent입니다. ROAS 4배 이상이면 예산을 늘려도 좋은 시점입니다.',
  },
  // 리드 품질
  {
    source: 'lead_quality',
  ...
  {
    source: 'kpi_analysis',
    title: '광고 성과 지표(KPI) 벤치마크 — 클릭은 많은데 구매가 없을 때, 경쟁사보다 효율이 낮을 때',
    content: 'CTR(클릭률): 이커머스 평균 1.5-2.5%, 뷰티 2.0-3.0%, 패션 1.8-2.8%. 클릭은 많은데 구매가 없다면(낮은 CVR) 랜딩 페이지나 가격을 점검하세요. CVR(전환율): 이커머스 2-4%, 뷰티 3-5%, SaaS 5-8%. CPA(전환 비용): 이커머스 15,000-30,000원, 뷰티 10,000-20,000원. ROAS: 이커머스 목표 3-5배, 뷰티 4-6배. ROAS 4배 이상이면 Excellent, 2.5배 이상이면 Good. 경쟁사보다 광고 효율이 낮다면 업종 벤치마크와 비교해보세요. 성과 급락 시 확인: 소재 피로도, 시즌 변동, 경쟁 심화, 추적 문제.',
  },
  // 카피라이팅 심리학
  {
