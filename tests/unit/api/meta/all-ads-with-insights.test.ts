...
import { GET } from '@/app/api/meta/all-ads-with-insights/route'
...
function createRequest(datePreset?: string): NextRequest {
    const url = datePreset
        ? `http://localhost:3000/api/meta/all-ads-with-insights?datePreset=${datePreset}`
        : 'http://localhost:3000/api/meta/all-ads-with-insights'
...
const mockAdSetsData = [
    { id: 'adset-1', name: 'AdSet 1', status: 'ACTIVE' },
]
const mockAdsData = [
...
describe('GET /api/meta/all-ads-with-insights - Bulk Optimized', () => {
    ...
    it('should return empty ads when getMetaAccountForUser returns null (token decrypt fail)', async () => {
    ...
    it('should return empty ads when getMetaAccountForUser returns null (empty token)', async () => {
        mockGetMetaAccountForUser.mockResolvedValue(null)
        const response = await GET(createRequest())
        expect(response.status).toBe(200)
        const body = await response.json()
    ...
    it('should return empty ads when no Meta account exists', async () => {
        mockGetMetaAccountForUser.mockResolvedValue(null)
        const response = await GET(createRequest())
        expect(response.status).toBe(200)
        const body = await response.json()
    ...
    it('should return empty ads when Meta account has no accessToken', async () => {
        mockGetMetaAccountForUser.mockResolvedValue(null)
        const response = await GET(createRequest())
        expect(response.status).toBe(200)
        const body = await response.json()
    ...
    it('should return empty ads when campaigns list is empty', async () => {
    ...
    it('should return ads with insights on success', async () => {
        const response = await GET(createRequest())
        expect(response.status).toBe(200)
        const body = await response.json()
