...
4. **Update `docs/plans/INDEX.md`**: "미구현 (Pending)" 테이블에 새 항목 추가
   - 형식: `| [feature-name](PLAN_<feature-name>.md) | <생성일> | <한줄 요약> | <규모> |`
   - INDEX.md가 없으면 생성 (아래 INDEX 템플릿 참조)
5. Inform user of plan location and next steps

#### INDEX.md 템플릿
INDEX.md가 없을 경우 아래 구조로 생성:
```markdown
# Plans Index
...
#### INDEX 업데이트 규칙
- **계획 생성 시**: "미구현" 섹션에 추가
- **구현 시작 시**: "미구현" → "진행 중"으로 이동
- **구현 완료 시**: "진행 중" → "완료"로 이동
