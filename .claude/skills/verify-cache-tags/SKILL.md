...
| `src/app/api/reports/[id]/send/route.ts` | 보고서 이메일 전송 API — 전달 전용 사이드이펙트 (ISR 태그 예외) |
| `src/app/api/reports/[id]/share/route.ts` | 보고서 공유 API — 전달/링크 생성 전용 (ISR 태그 예외) |
| `src/app/api/campaigns/budget-recommendation/route.ts` | 예산 추천 API — 계산 결과 반환 전용 (ISR 태그 예외) |
