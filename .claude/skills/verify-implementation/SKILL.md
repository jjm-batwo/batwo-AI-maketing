...
| # | 스킬 | 설명 |
|---|------|------|
| 1 | `verify-architecture` | 클린 아키텍처 레이어 의존성 규칙 검증 |
| 2 | `verify-di-registration` | DI 컨테이너 토큰-등록 동기화 검증 |
| 3 | `verify-cache-tags` | ISR 캐시 태그와 revalidateTag 매핑 일관성 검증 |
| 4 | `verify-bundle` | 번들 최적화 검증 (namespace import, dev-only 누출) |
