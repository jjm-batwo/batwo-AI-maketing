...
4. **강제 지점 일관성** — 암복호화 강제가 helper 호출 기반인지, repository mapper/middleware 기반인지를 명시하고 혼용 시 누락 경로를 점검
...
**PASS:** 결과 없음 또는 아래 강제 지점 중 하나가 확인됨

- 파일 단위 helper 사용(`encryptToken`, `safeDecryptToken`)
- repository mapper/middleware에서 중앙 강제 적용이 문서화되어 있고 실제 코드로 확인됨

**FAIL:** accessToken을 사용하지만 helper 사용/중앙 강제 적용 모두 확인되지 않음
