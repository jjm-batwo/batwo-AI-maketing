...
export function HybridTrackingCard({ pixelId }: HybridTrackingCardProps) {
    ...
    if (error || !health) {
        return (
            <div className="p-6 rounded-lg border border-red-200 bg-red-50 text-red-800">
                ...
                <p className="mt-1 text-sm text-red-600">트래킹 상태를 불러오지 못했습니다. 잠시 후 다시 시도해 주세요.</p>
            </div>
        )
    }
    ...
    const isHealthyDeduplication = capiEvents > 0 && browserEvents > 0 && health.matchRate && health.matchRate >= 0.6
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 divide-y md:divide-y-0 md:divide-x">
                ...
                <div className="p-6">
                    <div className="flex items-center gap-2 mb-4">
                        ...
                        <div>
                            <h4 className="font-semibold text-blue-900 border-b border-transparent">브라우저 픽셀</h4>
                    ...
                    <div className="space-y-4">
                        <div>
                            <span className="text-4xl font-bold text-slate-800">{browserEvents.toLocaleString()}</span>
                        ...
                        <div className="space-y-2">
                            <div className="flex justify-between text-sm">
                                ...
                                <span className="font-medium text-green-600">{health.matchedEventCount.toLocaleString()}</span>
                            </div>
                            <div className="flex justify-between text-sm">
                                <span className="text-slate-600">매칭 실패</span>
                                <span className="font-medium text-amber-600">{health.unmatchedEventCount.toLocaleString()}</span>
                            </div>
                        </div>
                    </div>
                </div>
                ...
                <div className="p-6">
                    ...
                    <div className="space-y-4">
                        <div>
                            <span className="text-4xl font-bold text-slate-800">{capiEvents.toLocaleString()}</span>
                        ...
                        <div className="space-y-2">
                            <div className="flex justify-between text-sm">
                                ...
                                <span className={cn('font-medium', health.capiEventsFailed > 0 ? 'text-red-500' : 'text-slate-600')}>
                                    {health.capiEventsFailed.toLocaleString()}
                                </span>
                            </div>
                            <div className="flex justify-between text-sm">
                                <span className="text-slate-600">시효 만료 (7일 초과)</span>
                                <span className={cn('font-medium', health.capiEventsExpired > 0 ? 'text-amber-500' : 'text-slate-600')}>
                ...
                <div className="mx-6 mb-6 p-4 bg-blue-50 border border-blue-200 rounded-lg flex gap-3">
                    ...
                    <div className="text-sm text-blue-800">
                        ...
                        <p className="text-blue-700/80">
                            만약 이 현상이 지속된다면 서버 환경(Node.js 혹은 플랫폼 환경)에서 전환 이벤트가 올바르게 트리거되고 있는지 인프라 코드를 점검해 보세요. 정상적인 하이브리드 세팅이라면 두 이벤트 수는 비슷하게 유지되며 Meta에서 <code className="bg-blue-100 px-1 py-0.5 rounded text-xs">event_id</code> 기반으로 중복을 제거합니다.
                        </p>
                    </div>
                </div>
            )}
