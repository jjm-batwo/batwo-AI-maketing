    ...
    analyze(input: AnalysisInput): DomainScore {
        ...
        if (frequency > 3.5) {
            ...
            factors.push({
                ...
                score: -40,
                weight: 0.6,
                explanation: '동일 타겟 대상 노출 빈도가 3.5회를 초과하여 CPA 19% 상승 페널티 위험이 매우 큽니다.',
            })
            ...
            factors.push({
                ...
                score: 0,
                weight: 0.4,
                explanation: '노출 빈도가 적절하게 관리되어 입찰 페널티 없이 탐색(Exploration)이 가능합니다.',
            })
        }
        ...
        if (entityIdCount < 4) {
            ...
            recommendations.push({
                domain: this.domain,
                priority: 'high',
                recommendation: '시각적 스타일(UGC, 스튜디오, 텍스트 그래픽 등)이 완전히 다른 최소 10개의 신규 소재 확보',
                scientificBasis: 'Andromeda 검색 인덱싱은 텍스트나 사소한 변경이 아닌, 완전히 다른 픽셀 패턴을 새로운 Entity ID로 인식함',
            ...
            factors.push({
                ...
                score: 0,
                weight: 0.2,
                explanation: '10개 이상의 다채로운 Entity ID를 통해 알고리즘 학습에 필요한 충분한 재료를 제공하고 있습니다.',
            })
        } else {
            factors.push({
                name: '적정 수준의 다양성',
                score: 0,
                weight: 0.3,
                explanation: '최소한의 Entity ID를 확보 중이나, 성과 확장을 위해 시각적 스타일 다양화를 권장합니다.',
            })
        }
