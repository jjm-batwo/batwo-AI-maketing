...
export class NeuromarketingAnalyzer implements DomainAnalyzer {
  ...
  private analyzeCognitiveLoad(
    ...
    const citation: Citation = {
      ...
      finding:
        '인지 부하 이론: 작업 기억은 3-5개 정보 단위만 동시에 처리 가능. 광고 카피는 12-20단어가 최적.',
  ...
  private analyzeEmotionalProcessing(allText: string): {
    ...
    const citation: Citation = {
      ...
      finding:
        '의사결정의 95%는 무의식적 감정 처리에서 발생. 감정을 자극하는 단어가 뇌의 편도체를 활성화하여 기억 강화.',
  ...
  private analyzeDopamineResponse(allText: string): {
    ...
    const citation: Citation = {
      ...
      finding:
        '도파민은 보상 자체보다 보상에 대한 기대감에서 더 많이 분비됨. 새로움은 도파민 분비를 30% 증가시킴.',
  ...
  private analyzeDualProcessAlignment(
    ...
    if (objective === 'awareness') {
      ...
      if (emotionalIntensity > 0.1 && emotionalIntensity < 0.2) score += 20 // Balance
    }
    ...
    const citation: Citation = {
      ...
      finding:
        '이중 처리 이론: System 1(빠른 직관)은 인지, System 2(느린 논리)는 고려/전환 단계에 적합.',
      ...
      if (factor.score < 70) {
        ...
        if (factor.name === '인지 부하 최적화') {
          recommendations.push({
            ...
            priority,
            recommendation:
              '광고 텍스트를 12-20단어로 줄이세요. 핵심 메시지 하나에 집중하고 부수적 정보는 제거하세요.',
            scientificBasis:
