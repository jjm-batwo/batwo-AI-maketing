...
NEVER: 모호한 "상황에 따라 다릅니다" 답변`,
      ...
      [ChatIntent.CREATIVE_FATIGUE]: `=== 크리에이티브 피로도 가이드 ===
사용자가 광고 효율 저하를 호소할 때, 피로도(빈도, 타겟 도달 등)를 점검하고 새로운 소재 교체를 권장합니다.
10개 이상의 텍스트/시각적으로 다른 Entity ID 확보를 제안하세요.`,

      [ChatIntent.LEARNING_PHASE]: `=== 머신러닝 학습 단계 가이드 ===
학습 단계(Learning Phase) 중단 또는 효율 감소 시 주 50회 전환 목표 달성을 강조합니다.
구조 통합 또는 예산 증액(최대 20%)을 제안하세요.`,

      [ChatIntent.STRUCTURE_OPTIMIZATION]: `=== 캠페인 구조 최적화 가이드 ===
복잡한 캠페인/광고 세트 구조를 Advantage+ 단일 캠페인으로 통합하도록 권고합니다.
계정 내 자기잠식(Cannibalization) 위험을 줄이는 방향으로 조언하세요.`,

      [ChatIntent.LEAD_QUALITY]: `=== 잠재고객 리드 품질 가이드 ===
무효 리드나 품질 저하 리드 발생 시, 인스턴트 폼에 조건부 로직이나 마찰(Friction) 항목 추가, 또는 CRM 데이터 CAPI 연동을 제안하세요.`,

      [ChatIntent.TRACKING_HEALTH]: `=== 트래킹 건전성 가이드 ===
전환 누락이나 데이터 불일치가 발생할 때 CAPI 연동 및 Event Match Quality(EMQ) 점수 6.0 이상 유지를 권장합니다.
쿠키리스 대응의 중요성을 어필하세요.`,
  ...
  private generateSuggestedQuestions(userMessage: string): string[] {
    ...
    const followUpMap: Record<ChatIntent, string[]> = {
      ...
      [ChatIntent.CREATIVE_FATIGUE]: [
        '최근 소재별 피로도 수치를 볼 수 있을까요?',
        '새로운 소재 레퍼런스가 필요합니다.',
        '내 인더스트리에 맞는 크리에이티브 전략이 있을까요?',
      ],
      [ChatIntent.LEARNING_PHASE]: [
        '현재 학습 단계에 머물러 있는 세트가 있나요?',
        '예산을 얼마나 올려야 학습을 마칠 수 있을까요?',
        '캠페인을 합치는 과정을 도와주세요.',
      ],
      [ChatIntent.STRUCTURE_OPTIMIZATION]: [
        '어떤 광고 세트들을 하나로 합치면 좋을까요?',
        'Advantage+ 쇼핑 캠페인으로 마이그레이션해주세요.',
        '계정 구조 진단 리포트를 생성해주세요.',
      ],
      [ChatIntent.LEAD_QUALITY]: [
        '어떻게 하면 진성 고객만 남길 수 있을까요?',
        '리드 폼 형식을 수정하고 싶어요.',
        'CRM 데이터 연동 방법을 알려주세요.',
      ],
      [ChatIntent.TRACKING_HEALTH]: [
        '현재 EMQ 점수는 몇 점인가요?',
        'CAPI 설정 가이드를 보내주세요.',
        '전환 이벤트가 모두 정상 수집되고 있나요?',
      ],
      [ChatIntent.GENERAL]: [
        '이번 주 성과는 어때?',
        '새 캠페인을 만들어줘',
