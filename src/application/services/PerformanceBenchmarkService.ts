...
export class PerformanceBenchmarkService {
  ...
  private interpolatePercentile(
    ...
    const thresholds = isInverse ? { p25: -p.p25, p50: -p.p50, p75: -p.p75, p90: -p.p90 } : p

    if (v >= thresholds.p90) return 95
    if (v >= thresholds.p75)
      return 75 + (15 * (v - thresholds.p75)) / (thresholds.p90 - thresholds.p75)
    if (v >= thresholds.p50)
      return 50 + (25 * (v - thresholds.p50)) / (thresholds.p75 - thresholds.p50)
    if (v >= thresholds.p25)
      return 25 + (25 * (v - thresholds.p25)) / (thresholds.p50 - thresholds.p25)
