...
const paramsSchema = z.object({
  questionId: z
    .string()
    .describe('질문 식별자 (experience_level, industry, objective, budget, target)'),
...
export function createAskGuideQuestionTool(): AgentTool<Params, GuideQuestionResult> {
  return {
    ...
    description:
      '캠페인 가이드 질문을 표시합니다. 사용자의 경험 수준과 비즈니스 정보를 파악하기 위한 인터뷰 질문에 사용합니다.',
    parameters: paramsSchema,
    requiresConfirmation: false,

    async execute(params: Params): Promise<ToolExecutionResult<GuideQuestionResult>> {
      const optionLines = params.options.map(
