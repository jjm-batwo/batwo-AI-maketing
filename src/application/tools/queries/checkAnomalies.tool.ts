...
const paramsSchema = z.object({
  campaignId: z
    ...
    .optional()
    .describe(
      '캠페인 ID 또는 캠페인 이름 (미지정 시 전체 캠페인 확인). 정확한 ID를 모르면 캠페인 이름을 전달해도 됩니다.'
    ),
  metric: z
    .enum(['spend', 'ctr', 'roas', 'conversions', 'all'])
    .default('all')
    .describe('확인할 지표'),
})
...
export function createCheckAnomaliesTool(
  ...
  return {
    name: 'checkAnomalies',
    description:
      '캠페인 성과 지표에서 이상 징후를 감지합니다. 급격한 변화나 비정상적인 패턴을 찾아냅니다.',
    ...
    async execute(params: Params, context: AgentContext): Promise<ToolExecutionResult> {
      ...
      if (params.campaignId) {
        ...
        if (byId) {
          ...
          const byName = allCampaigns.filter(
            (c) =>
      ...
      for (const campaign of campaigns) {
        ...
        const previousAgg = await kpiRepository.aggregateByCampaignId(
          campaign.id,
          sevenDaysAgo,
          threeDaysAgo
        )
        ...
        if (params.metric === 'all' || params.metric === 'spend') {
          if (previousAgg.totalSpend > 0) {
            const spendChangeRate =
              ((recentAgg.totalSpend - previousAgg.totalSpend) / previousAgg.totalSpend) * 100
        ...
        if (params.metric === 'all' || params.metric === 'ctr') {
          const recentCtr =
            recentAgg.totalImpressions > 0
              ? (recentAgg.totalClicks / recentAgg.totalImpressions) * 100
              : 0
          const previousCtr =
            previousAgg.totalImpressions > 0
        ...
        if (params.metric === 'all' || params.metric === 'roas') {
          const recentRoas =
            recentAgg.totalSpend > 0 ? recentAgg.totalRevenue / recentAgg.totalSpend : 0
          const previousRoas =
            previousAgg.totalSpend > 0 ? previousAgg.totalRevenue / previousAgg.totalSpend : 0
      ...
      if (anomalies.length === 0) {
        return {
          ...
          data: { anomalies: [] },
          formattedMessage:
            '현재 감지된 이상 징후가 없습니다. 모든 캠페인이 정상 범위 내에서 운영 중입니다.',
        }
      }

      const lines = anomalies.map((a) => `- [${a.severity}] ${a.campaignName}: ${a.message}`)
