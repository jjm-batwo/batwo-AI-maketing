...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    const userAbTests = abTests.filter((test) => userCampaignIds.has(test.campaignId))
...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    // Build variants
    const abTestVariants: ABTestVariant[] = variants.map(
      (v: { name: string; description?: string; trafficPercent: number; isControl: boolean }) => ({
